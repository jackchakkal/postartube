...
import { ChannelConfig, VideoSlot, VideoStatus, Language, ChannelProfile, DbProfile, Platform, PLATFORM_CONFIG } from './types';
...
import { profilesService, slotsService, userConfigService, diagnosticService } from './services/database';
import { format } from 'date-fns';
import { Zap, Download, RotateCcw, Moon, Sun, FileText, FileSpreadsheet, Users, ChevronDown } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (mounted) {
          if (data?.session) {
            ...
            const configResult = await userConfigService.get(data.session.user.id);
            if (configResult.data) {
              setLang(configResult.data.language as Language);
              setDarkMode(configResult.data.theme === 'dark');
            }

            // Executar diagnóstico em background
            diagnosticService.runFullDiagnostic(data.session.user.id);
          } else {
    ...
    const timeout = setTimeout(() => {
      if (mounted && sessionLoading) {
        console.warn('App: Timeout de carregamento');
        setSessionLoading(false);
      }
    }, 5000);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, newSession) => {
      ...
      if (newSession) {
        const configResult = await userConfigService.get(newSession.user.id);
        if (configResult.data) {
          setLang(configResult.data.language as Language);
          setDarkMode(configResult.data.theme === 'dark');
        }
  ...
  // Carregar perfis quando sessão mudar
  useEffect(() => {
    if (session) {
      loadProfiles();
    }
  ...
  // Carregar slots quando perfil ou data mudar
  useEffect(() => {
  ...
  const loadProfiles = async () => {
    ...
    if (result.error) {
      console.error('Erro ao carregar perfis:', result.error);
      return;
    }

    if (result.data) {
      const mapped: ChannelProfile[] = result.data.map((p: DbProfile) => ({
        id: p.id,
        name: p.name,
        platform: p.platform,
        config: {
  ...
  const loadSlots = async () => {
    ...
    const result = await slotsService.getByProfileAndDate(activeProfileId, dateStr);

    if (result.data) {
      const mapped: VideoSlot[] = result.data.map((s) => ({
        id: s.id,
        time: s.time,
        type: s.type,
        topic: s.topic || '',
  ...
  const toggleTheme = () => {
    ...
    setDarkMode(newVal);
    if (session) {
      userConfigService.upsert(session.user.id, { theme: newVal ? 'dark' : 'light' });
    }
  };
  ...
  const changeLang = (l: Language) => {
    setLang(l);
    if (session) {
      userConfigService.upsert(session.user.id, { language: l });
    }
  };
  ...
  const handleConfigChange = async (newConfig: ChannelConfig) => {
    ...
    await profilesService.update(activeProfileId, {
  ...
  const handleCreateProfile = async (name: string, platform: Platform) => {
    ...
    console.log('Criando perfil:', { name, platform });

    const result = await profilesService.create(session.user.id, name, platform);
  ...
  const handleDeleteProfile = async (id: string) => {
    const result = await profilesService.delete(id);
  ...
  const generateSchedule = async () => {
    ...
    // 1. Limpar slots existentes
    const deleteResult = await slotsService.deleteByProfileAndDate(activeProfileId, dateStr);
    if (deleteResult.error) {
      alert(`Erro ao limpar agenda: ${deleteResult.error}`);
      setLoadingData(false);
      return;
    }

    // 2. Criar novos slots
    const newSlots = randomTimes.map(timeMins => ({
      profile_id: activeProfileId,
      date: dateStr,
      time: minsToTime(timeMins),
      type: config.videoType as 'LONG' | 'SHORT',
    ...
    const createResult = await slotsService.createMany(newSlots);
    if (createResult.error) {
      alert(`Erro ao criar agenda: ${createResult.error}`);
    }
  ...
  const updateSlot = async (id: string, updates: Partial<VideoSlot>) => {
    ...
    await slotsService.update(id, dbUpdates);
  };
  ...
  const clearSchedule = async () => {
    if (window.confirm(t.confirmClear)) {
      const dateStr = format(selectedDate, 'yyyy-MM-dd');
      await slotsService.deleteByProfileAndDate(activeProfileId, dateStr);
      setSlots([]);
    }
  };
  ...
  if (sessionLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 text-slate-500 gap-4">
        ...
        <p className="font-medium text-slate-700 dark:text-slate-300">Carregando...</p>
        <button
